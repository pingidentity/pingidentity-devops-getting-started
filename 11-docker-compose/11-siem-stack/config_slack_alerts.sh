...
if [[ $SLACK_WEB_HOOK != "" ]] 
  then
	...
	docker exec es02 /bin/sh -c ". /usr/share/elasticsearch/bootstrap/keystore_bootstrap.sh $SLACK_WEB_HOOK"
  	...
  	echo "!! Webhook was not provided, not updating the config. The existing webhook config will persist !!."
fi
...
if [[ $EP != "" ]] 
  then
	...
	do	
  	...
  	echo "!! Elastic Password was not provided, watcher configuration will persist."
fi

echo "Execution Complete. Review above logs to validate configuration loaded. Cheers ~Ping Team!"