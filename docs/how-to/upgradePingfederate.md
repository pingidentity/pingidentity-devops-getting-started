...
helm upgrade --install pf-upgrade pingidentity/ping-devops \
   --version 0.8.4 -f 20-kustomize/15-pingfederate-upgrade/01-background.yaml
```

where values.yaml:

```yaml
--8<-- "20-kustomize/15-pingfederate-upgrade/01-background.yaml"
```
...
The steps for upgrading can be automated with a script. Example scripts are included at `20-kustomize/15-pingfederate-upgrade/hooks`.

To use the scripts, copy the folder your PingFederate container

```
kubectl cp 20-kustomize/15-pingfederate-upgrade/hooks pf-upgrade-pingfederate-admin-0:/opt/staging
```
...
helm upgrade --install pf-upgrade pingidentity/ping-devops --version 0.8.4 \
   -f 20-kustomize/15-pingfederate-upgrade/02-upgraded.yaml
```

```yaml
--8<-- "20-kustomize/15-pingfederate-upgrade/02-upgraded.yaml"
```

This will restart the admin console, and trigger a rolling update of all the engines.

!!! Info "Old Profile"
The final yaml `20-kustomize/15-pingfederate-upgrade/02-upgraded.yaml` still points to the same profile. The steps that should have been completed in [Server Profile Upgrade](#server-profile-upgrade) were not included.
